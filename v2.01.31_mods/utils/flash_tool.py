...
if len(argv) == 5: arg_nblocks = int(argv[4],0)
...
with libuvk5.uvk5(arg_port) as radio:
    if path.exists(arg_file):
        if radio.connect():
            radio.get_fw_version()
            reply = radio.rom_flash_set('*.01.31')
            if not reply: print('\nERR: No response. Make sure transceiver is in correct boot mode!');exit(1)
                        ...
                        for i in range(offset,end,256):
                            ...
                            if SAFE:
                                ...
                                while reply[4:6] != b'\x1A\x05':
                                    ...
                                    if int(time()) - wd == 10: print('ERROR: Timeout while waiting for ACK'); exit(1)
    ...
    else: print('ERROR: File not found');exit(1)
